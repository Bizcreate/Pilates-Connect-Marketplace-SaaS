...
export function StudioReferralWidget() {
  ...
  useEffect(() => {
    ...
    async function loadReferralData() {
      ...
      if (profileError) {
        console.error("[v0] StudioReferralWidget: Error fetching profile:", profileError)
      ...
      if (referralError) {
        console.error("[v0] StudioReferralWidget: Error fetching referrals:", referralError)
        return
      }
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>Earn $50 for each studio you refer who posts their first job</CardDescription>
          </div>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Share this link with other Pilates studios. You'll earn $50 when they post their first job!
          </p>
        </div>
              ...
              {referrals.map((referral) => (
                <div key={referral.id} className="flex items-center justify-between p-2 border rounded-lg text-sm">
                  <div className="flex-1 min-w-0">
                    <p className="font-medium truncate">{referral.referred_name || "New Studio"}</p>
                    <p className="text-xs text-muted-foreground">
